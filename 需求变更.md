...
> 强制底线见 [`AGENTS.md`](./AGENTS.md)。本文讲开发人员怎么操作、执行什么命令、产出什么。从零开发新需求见 [`新需求开发.md`](./新需求开发.md)。
>
> **核心原则：不急着新建——先定位原记录、判断变化等级、把正文更新为当前口径、追加变化记录（不覆盖历史），再改代码。**
>
> **文档只有一处事实来源——OpenSpec 的 change 目录。** Superpowers 为辅，只贡献"方法"，不另产文档：`$brainstorming` 的澄清结论回填到 change 的 `proposal` / `specs`，`$writing-plans` 的拆解结果落进 `tasks.md`，都不要再单独生成 `docs/superpowers/specs/` 下的设计稿或独立计划文件。

## 先搞清楚三种调用方式

| 类型 | 写法 | 在哪里用 | 例子 |
| --- | --- | --- | --- |
| **AI 技能**（Superpowers） | `$技能名` | AI 对话框里直接打 | `$brainstorming`、`$writing-plans` |
| **AI 斜杠命令**（OpenSpec） | `/opsx:命令 参数` | AI 对话框里直接打 | `/opsx:continue update-password-policy` |
| **终端 CLI**（OpenSpec） | `openspec 子命令` | 项目根目录的终端里 | `openspec list`、`openspec show <change-name>` |

约定：尖括号 `<change-name>` 是占位符，替换成真实值（小写短横线，如 `update-password-policy`）。**基础工作流**（`/opsx:propose`、`/opsx:apply`、`/opsx:sync`、`/opsx:archive`）`openspec init` 后即可用；**扩展工作流**（`/opsx:new`、`/opsx:continue`、`/opsx:ff`、`/opsx:verify`）需先配 profile 并 `openspec update`，不可用就退回基础命令或直接让 AI 改 change 文档。
...
```text
/opsx:apply update-password-policy        # 实现，配合 $test-driven-development
...
把 change 换成对应的 issue、PR 或外部需求记录：同样先定位原记录、更新正文为当前口径、追加变化记录、同步验收，再改代码。CodeGraph 不可用就人工分析。`AGENTS.md` 底线不打折。
