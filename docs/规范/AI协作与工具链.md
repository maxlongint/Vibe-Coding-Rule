...
> **适用范围：** 采用 Vibe Coding 四件套（OpenSpec、CodeGraph、Superpowers、项目技能包）或其中部分工具的项目。未安装某工具时，按 `AGENTS.md` 强制底线 fallback 到人工分析、轻量记录或向用户确认。
>
...
3. 如任务涉及行为、接口、数据、权限、流程或验收标准变化，必须先检查或创建 OpenSpec change；如项目尚未初始化 OpenSpec，应先向用户确认初始化方式或外部需求记录。
4. 如任务涉及代码修改、影响判断、调用链、重构或缺陷定位，应优先使用 CodeGraph 分析现有代码。
5. 根据任务类型启用对应 Superpowers 流程。
...
- 用户明确要求沉淀真实业务需求时，默认写入 OpenSpec change；若用户明确使用外部需求系统，应在对应记录中保留外部链接或编号。
