...
[![Version: v4.2.1](https://img.shields.io/badge/Version-v4.2.1-blue)](./CHANGELOG.md)
...
普通问答、只读分析、仓库状态检查和用户直接授权的小改动，按当前请求处理，不自动创建或查找 OpenSpec；若实施中发现会改变外部行为、接口、数据、权限、流程或验收，先停止并确认承接方向。

只有用户明确指定新增需求、需求变更或已有需求的后续工作，才进入 OpenSpec 承接流程；用户未指定时，AI 按直接授权的任务处理。确认后的目标、范围、设计、任务与验证结论回填同一 change，issue/PR 等只作输入来源。

完成实现或验证后，应说明实际改变、已核对的不变式、验证证据、未验证项和剩余风险；涉及外部状态时说明授权来源和实际结果。
...
实际任务涉及既有代码结构、符号关系、引用、调用路径或影响范围时，先确认 CodeGraph 已完成当前项目索引且当前会话可用，再用 CodeGraph 取得证据；CodeGraph 不可用时，中断依赖代码图谱证据的部分，不用纯文本搜索或模型猜测冒充同等结论。
