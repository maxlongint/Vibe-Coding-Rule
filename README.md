...
### 6.7 项目级技能包：`$update-requirement-docs`

本仓库还提供了一个手动调用型需求文档更新技能包：

```text
.agents/skills/update-requirement-docs/SKILL.md
```

它只在用户明确点名时使用，用于处理已有需求的补充、范围变化、验收调整或 OpenSpec change 同步。普通需求讨论、规则问答或临时想法不会自动触发该技能。

推荐调用方式：

```text
请使用 $update-requirement-docs 更新 openspec/changes/<change-name>/。
这次需求新口径是：...
请保留原需求变化记录，并同步 proposal、design、tasks 和 spec。
```

这个技能的核心规则是：需求正文更新为当前有效口径，需求变化记录保留时间、来源、原口径、新口径、影响范围、验收变化和剩余风险，避免直接覆盖历史。
