...
同一个任务、同一个 bug 或同一批文件，可以继续使用同一个 Chat；新话题、新需求或新方向，另开 Chat 更干净。任务明显结束后，后续无关工作不要硬续；复杂任务可以更长，不需要机械地每 3 到 6 轮就切换。

为了节省 token，只提供相关文件、相关片段和关键报错，不把整仓库、整段历史或整篇日志塞进上下文。新 Chat 不需要复制全部旧对话，用 2 到 5 句摘要交代“当前状态、已排除方案、下一步目标”最有用。

长对话会增加模型需要处理的上下文；Prompt Cache 可以省钱和提速，但不代表上下文免费。上下文越长，模型越容易分心，响应也可能更慢；大上下文窗口只是容量更大，不代表塞越多越好。

常见误区：

- “一个 Chat 一定最省 token”：不一定，长历史会反复带来成本。
- “上下文看着不计费”：错，输入上下文通常会计入 token，只是可能有缓存折扣。
- “轮数越多越划算”：不一定，越长越容易贵、慢、乱。
- “每 3 到 6 轮必须新开”：也不对，要看任务是否仍然连续。

一句话口诀：同事续聊，新事新开；只给关键，不搬全文；摘要接力，别拖长尾。
