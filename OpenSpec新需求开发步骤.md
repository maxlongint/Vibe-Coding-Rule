...
本文说明在已启用 OpenSpec 的项目中，如何用 OpenSpec change 推进一个新需求。强制规则见 `AGENTS.md` 第 1 节「文档目录与需求记录结构」和第 8 节「AI 执行规则」；与 Superpowers 的流程对照见 `Superpowers与OpenSpec使用指南.md`；复杂需求完整示例见 `SSO 单点登录复杂需求执行步骤示例.md`。
...
如果项目还没有 `openspec/`，**不要默认直接初始化**。先按 `AGENTS.md` 第 1 节确认需求记录载体：
...
3. 如果不存在，**先向用户确认需求记录载体**（OpenSpec、外部系统或轻量记录），见 `AGENTS.md` 第 1 节；用户确认 OpenSpec 后再初始化，不得默认直接 `openspec init` 或跳过治理底线。
