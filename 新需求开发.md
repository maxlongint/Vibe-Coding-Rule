...
>
> **文档只有一处事实来源——OpenSpec 的 change 目录。** Superpowers 为辅，只贡献"方法"，不另产文档：`$brainstorming` 的澄清结论回填到 `proposal` / `specs`，`$writing-plans` 的拆解结果落进 `tasks.md`，都不要再单独生成 `docs/superpowers/specs/` 下的设计稿或独立计划文件。
...
- 尖括号 `<change-name>` 是**占位符**，要替换成你的真实值。`change-name` 用小写加短横线，能一眼看懂做什么，例如 `add-sso-login`、`update-password-policy`。
...
让 AI 按 `tasks.md` 逐项推进，**先写失败测试再写实现**，每完成一项就更新状态，你逐项审核。不在生产链路用 mock、默认值、空对象伪造成功。
...
把 change 换成 issue、任务卡、PR 描述或一份轻量记录，承载同样的产出：需求与验收、影响分析、任务与验证。CodeGraph 不可用就人工分析，Superpowers 没装就手动完成澄清、先测后改、完成前验证。至少写清变更范围、验证方式、剩余风险。`AGENTS.md` 底线不打折。
