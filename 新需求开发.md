...
- `openspec/changes/add-sso-login/tasks.md`——初始任务清单与验证方向。
...
```text
/opsx:ff add-sso-login        # 快进：一口气补方案并校准 tasks，适合需求清楚、想快点过
...
openspec/changes/add-sso-login/
  ...
  tasks.md               # 第 3 步创建；第 5 步按影响分析校准
  design.md              # 第 5 步按需补充：技术方案、数据流、安全策略、回滚
```
