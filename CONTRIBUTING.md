...
- **分场景、逐级授权**：新增需求、需求变更或已有需求的继续工作只能由用户明确指定；AI 不根据实现、改动范围、技术类别或可观察结果自行分类、要求确认或启动 OpenSpec。新增需求在口径确认后由用户主动授权创建，AI 生成 ID 并检查冲突；需求变更或已有需求仍须确认继续原 change / 新建关联 change 的方向，新建分支在口径确认后由创建请求授权生成 ID 和检查冲突。完成对应授权前不得写入 change、形成最终设计与计划或开始实施。
- **前后端通用**：不写特定框架实现细节。
