...
- **分场景、逐级授权**：AI 可以识别内容可能构成新增需求或需求变更，但需求意图确认前不得进入 OpenSpec 流程。新增需求在口径确认后由用户主动授权创建，AI 生成 ID 并检查冲突；需求变更或已有需求仍须确认继续原 change / 新建关联 change 的方向，新建分支在口径确认后由创建请求授权生成 ID 和检查冲突。完成对应授权前不得写入 change、形成最终设计与计划或开始实施。
- **前后端通用**：不写特定框架实现细节。
